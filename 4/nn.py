...
'''
指数衰减：learning_rate = learning_rate*0.9^t
+
1/t衰减：learning_rate = learning_rate/(1+0.1*t)
'''
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for x, y in tqdm(train_loader):
            ...
            # 更新参数
            model1.update(learning_rate*(0.9**epoch))
            ...
            # 更新参数
            model2.update(learning_rate/(1+0.1*epoch))
            step += 1

    plt.plot(torch.arange(1, step), loss_history1, label='Exponetial Annealing')
    plt.plot(torch.arange(1, step), loss_history2, label='1/t Annealing')
    plt.xlabel("steps")
    plt.ylabel("loss")
    plt.legend()
