...
class MyLinear(nn.Module):
    def __init__(self, input_dim, output_dim, regularize=None):
    ...
    def backward(self, grad_output):
        ...
        if self.regularize == 'L1':
            self.gradW = self.input.T @ grad_output + l * torch.sgn(self.W)
            self.gradb = grad_output.sum(dim=0) + l * torch.sgn(self.b)
        else:
            self.gradW = self.input.T @ grad_output + 2 * l * self.W
            self.gradb = grad_output.sum(dim=0) + 2 * l * self.b
...
class MyModel(nn.Module):
    def __init__(self, num_feature, num_class, hidden_dim, regularize=None):
        super(MyModel, self).__init__()
        self.linear1 = MyLinear(num_feature, hidden_dim, regularize)
        self.sigmoid1 = MySigmoid()
        self.linear2 = MyLinear(hidden_dim, num_class, regularize)
...
if __name__ == '__main__':
    ...
    # init
    model1 = MyModel(28*28, 12, 80, regularize="L1")
    model1.to(device)
    ...
    plt.plot(torch.arange(1, step), loss_history1, label='L1 Regularization')
    plt.plot(torch.arange(1, step), loss_history2, label='L2 Regularization')
    plt.xlabel("steps")
    plt.ylabel("loss")
    plt.legend()
